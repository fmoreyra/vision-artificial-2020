...
def draw(event, x, y, flags, param):
    global first_point_x, first_point_y, second_point_x, second_point_y, third_point_x, third_point_y, count, drawing
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        count = count + 1
        cv2.circle(background_image, (x, y), 5, (0, 0, 255), -1)
...
while (True):
    ...
    if count is 3:
        ...
        matrix = cv2.getAffineTransform(added_image_limit_points, background_points)

        # Transformed added image (in color)
        warped_added_image = cv2.warpAffine(added_image, matrix, (background_image.shape[1], background_image.shape[0]))
